    ...
    if (error || !order) {
        return (
            <Box className="p-12 text-center">
                <Typography variant="h6" className="text-red-500 font-bold">{error || "Sipariş bulunamadı."}</Typography>
            ...
            <Box className="flex flex-wrap justify-between items-end gap-4 bg-white dark:bg-white/5 p-6 rounded-xl border border-[#e1e4e2] dark:border-white/10 shadow-sm">
                <Box className="flex min-w-72 flex-col gap-2">
                    <Box className="flex items-center gap-3">
                        <h1 className="text-text-dark dark:text-white text-3xl font-black leading-tight tracking-tight">Sipariş Detayları</h1>
                    ...
                    <p className="text-text-muted dark:text-white/60 text-base font-normal">
                        Sipariş No: #{order.order_number} | {formatDate(order.created_at)}
                    </p>
                </Box>
                <Box className="flex gap-3">
                    ...
                    <button className="flex items-center gap-2 rounded-xl h-11 px-6 bg-primary text-white text-sm font-bold hover:opacity-90 transition-opacity shadow-lg shadow-primary/20">
                ...
                <Box className="grid grid-cols-1 md:grid-cols-4 gap-4 relative">
                    <Box className="flex flex-col items-center md:items-start gap-2 relative z-10">
                        ...
                        <Box className="flex flex-col items-center md:items-start">
                            <p className="text-text-dark dark:text-white text-sm font-bold">Sipariş Alındı</p>
                    ...
                    <Box className="flex flex-col items-center md:items-start gap-2 relative z-10">
                        ...
                        <Box className="flex flex-col items-center md:items-start">
                            <p className="text-text-dark dark:text-white text-sm font-bold">Hazırlanıyor</p>
                        </Box>
                    </Box>
                    <Box className="flex flex-col items-center md:items-start gap-2 relative z-10">
                        ...
                        <Box className="flex flex-col items-center md:items-start">
                            <p className={`text-sm font-black ${order.status?.toLowerCase() === 'shipped' ? 'text-primary' : 'text-text-dark dark:text-white'}`}>Kargoya Verildi</p>
                        </Box>
                    </Box>
                    <Box className={`flex flex-col items-center md:items-start gap-2 relative z-10 ${order.status?.toLowerCase() === 'delivered' ? '' : 'opacity-40'}`}>
                        ...
                        <Box className="flex flex-col items-center md:items-start">
                            <p className="text-text-dark dark:text-white text-sm font-bold">Teslim Edildi</p>
                            <p className="text-text-muted dark:text-white/60 text-xs">Tahmini 3-5 İş Günü</p>
                        </Box>
                    </Box>
            ...
            <Box className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <Box className="lg:col-span-2 flex flex-col gap-4">
                    <Box className="flex items-center justify-between">
                        <h2 className="text-text-dark dark:text-white text-[22px] font-bold tracking-tight">Siparişteki Ürünler ({order.items?.length})</h2>
                    </Box>
                        ...
                        <Box key={item.id} className="group bg-white dark:bg-white/5 p-4 rounded-xl border border-[#e1e4e2] dark:border-white/10 flex gap-4 hover:shadow-md transition-shadow">
                            ...
                            <Box className="flex flex-col justify-between flex-1">
                                <Box className="flex justify-between items-start">
                                    ...
                                    <p className="text-text-dark dark:text-white font-black">₺{parseFloat(item.unit_price).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</p>
                                </Box>
                                <Box className="flex items-center justify-between mt-2">
                                    <p className="text-text-muted dark:text-white/60 text-sm">Adet: <span className="font-bold text-text-dark dark:text-white">{item.quantity}</span></p>
                                    <button className="text-primary text-xs font-black hover:underline">Ürünü Değerlendir</button>
                                </Box>
                            </Box>
                        </Box>
                    ))}
                ...
                <Box className="flex flex-col gap-6 sticky top-24">
                    ...
                    <Box className="bg-white dark:bg-white/5 p-6 rounded-xl border border-[#e1e4e2] dark:border-white/10 flex flex-col gap-4 shadow-sm">
                        <Box className="flex items-center gap-2">
                            ...
                            <h4 className="text-text-dark dark:text-white font-bold text-base">Teslimat Bilgileri</h4>
                        </Box>
                        ...
                        <Box className="flex items-center gap-2">
                            ...
                            <h4 className="text-text-dark dark:text-white font-bold text-base">Ödeme Yöntemi</h4>
                        </Box>
                        <Box className="flex items-center gap-3">
                            <Box className="w-10 h-6 bg-background-light dark:bg-white/10 rounded border border-gray-200 dark:border-white/10 flex items-center justify-center">
                                <span className="text-[10px] font-bold italic">CARD</span>
                            </Box>
                            <p className="text-text-dark dark:text-white text-sm font-medium">Banka/Kredi Kartı</p>
                        </Box>
                    </Box>
                    ...
                    <Box className="bg-white dark:bg-white/5 p-6 rounded-xl border border-[#e1e4e2] dark:border-white/10 flex flex-col gap-3 shadow-sm">
                        <h4 className="text-text-dark dark:text-white font-bold text-base pb-2">Sipariş Özeti</h4>
                        <Box className="flex justify-between items-center text-sm">
                            <span className="text-text-muted dark:text-white/60">Ara Toplam</span>
                            <span className="text-text-dark dark:text-white font-medium">₺{parseFloat(order.subtotal).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
                        </Box>
                        <Box className="flex justify-between items-center text-sm">
                            <span className="text-text-muted dark:text-white/60">Kargo Ücreti</span>
                            <span className={`font-bold ${parseFloat(order.shipping_cost) === 0 ? 'text-primary' : 'text-text-dark'}`}>
                                {parseFloat(order.shipping_cost) === 0 ? 'ÜCRETSİZ' : `₺${parseFloat(order.shipping_cost).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`}
                            </span>
                        </Box>
                        <Box className="mt-4 pt-4 border-t border-[#e1e4e2] dark:border-white/10 flex justify-between items-center">
                            <span className="text-text-dark dark:text-white font-black text-lg">Toplam</span>
                            <span className="text-accent font-black text-2xl tracking-tighter">₺{parseFloat(order.total_amount).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
                        </Box>
