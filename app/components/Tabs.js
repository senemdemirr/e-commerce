    ...
    <div className="tabs-container pt-2 w-100 mx-auto border-gray-200 border">
      <div className="tabs-header flex w-full justify-between">
        <button
          ...
          className={`tab w-50 px-4 py-2 ${tabIndex === 0 ? "font-bold border-b-2 border-orange-500" : ""}`}
        >
          Sign In
        </button>
        <button
          onClick={() => handleTabChange(1)}
          className={`tab w-50 px-4 py-2 ${tabIndex === 1 ? "font-bold border-b-2 border-orange-500" : ""}`}
        >
          Sign Up
        </button>
      </div>
