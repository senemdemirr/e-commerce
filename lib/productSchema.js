...
function parseLegacyDetails(value) {
    ...
    return {
        ...
        care: Array.isArray(value.care)
            ? value.care.map((item) => String(item || '').trim()).filter(Boolean)
            : [],
        bullet_points: Array.isArray(value.bullet_points)
            ? value.bullet_points.map((item) => String(item || '').trim()).filter(Boolean)
            : [],
        description_long: typeof value.description_long === 'string' ? value.description_long : '',
    };
}
...
function hasDetailContent(details) {
    return Boolean(
        details.material
        || details.description_long
        || details.care.length > 0
        || details.bullet_points.length > 0
    );
}
...
async function createTargetTables(client) {
    ...
    await client.query('DROP TABLE IF EXISTS details_next');
...
async function createNormalizedTablesIfMissing(client, colorsColumns, sizesColumns, detailsColumns) {
...
async function ensureProductsColumns(client, productColumns) {
    ...
    if (!productColumns.has('detail_id')) {
        await client.query('ALTER TABLE products ADD COLUMN detail_id INT[] NOT NULL DEFAULT ARRAY[]::INT[]');
    } else if (productColumns.get('detail_id') !== '_int4') {
        await client.query(`
            ALTER TABLE products
...
function buildDetailsObjectFromRows(rows) {
    const details = {
        material: '',
        care: [],
        bullet_points: [],
    ...
    for (const row of rows) {
        const detailKey = String(row.detail_key || '').trim();
        const detailValue = String(row.detail_value || '').trim();

        if (!detailKey || !detailValue) {
            continue;
        }

        if (detailKey === 'material') {
            details.material = detailValue;
        } else if (detailKey === 'care') {
            details.care.push(detailValue);
        } else if (detailKey === 'bullet_point') {
            details.bullet_points.push(detailValue);
        } else if (detailKey === 'description_long') {
            details.description_long = detailValue;
        }
    }
...
async function fetchDetailsByRefs(client, refIds, detailsColumns) {
    ...
    if (normalizedRefs.length === 0) {
        return parseLegacyDetails(null);
    }
    ...
    if (detailsColumns.has('material')) {
        ...
        return result.rowCount === 0
            ? parseLegacyDetails(null)
            : parseLegacyDetails(result.rows[0]);
    }

    if (detailsColumns.has('detail_key')) {
        const result = await client.query(
            `
...
async function fetchGroupDetails(client, detailRef, hasTable) {
    if (!hasTable || !detailRef) {
        return parseLegacyDetails(null);
    }
...
async function buildProductSnapshots(client, metadata) {
    ...
    for (const product of products) {
        ...
        const legacyDetails = metadata.productColumns.has('details')
            ? parseLegacyDetails(product.details)
            : parseLegacyDetails(null);
        const currentDetails = metadata.detailsTableExists
            ? await fetchDetailsByRefs(client, product.detail_id, metadata.detailsColumns)
            : parseLegacyDetails(null);
        const groupDetails = metadata.hasProductDetails
            ? await fetchGroupDetails(client, normalizeIdRefs(product.detail_id)[0] || product.detail_id, metadata.hasProductDetails)
            : parseLegacyDetails(null);
        ...
        snapshots.push({
            ...
            details: hasDetailContent(legacyDetails)
                ? legacyDetails
                : hasDetailContent(currentDetails)
                    ? currentDetails
                    : groupDetails,
            variants,
        });
...
async function insertDetailRows(client, details) {
    const detailIds = [];
    ...
    const insertDetail = async (detailKey, detailValue, displayOrder = 0) => {
        ...
        const result = await client.query(
            `
                INSERT INTO details_next (detail_key, detail_value, display_order)
                VALUES ($1, $2, $3)
                RETURNING id
            `,
...
async function populateNormalizedSchema(client, snapshots) {
    ...
    for (const snapshot of snapshots) {
        ...
        const detailIds = await insertDetailRows(client, snapshot.details);
        ...
        await client.query(
            `
                ...
                WHERE id = $4
            `,
            [colorIds, sizeIds, detailIds, snapshot.productId]
        );
...
async function replaceLookupTables(client, colorsTableExists, sizesTableExists, detailsTableExists) {
    if (colorsTableExists) {
    ...
    await client.query('ALTER TABLE details_next RENAME TO details');
...
function isNormalizedSchema(metadata) {
    return metadata.colorsColumns.has('name')
        ...
        && metadata.detailsColumns.has('detail_key')
        && metadata.detailsColumns.has('detail_value')
        && !metadata.detailsColumns.has('material')
        && metadata.productColumns.get('colors_id') === '_int4'
        && metadata.productColumns.get('sizes_id') === '_int4'
        && metadata.productColumns.get('detail_id') === '_int4'
...
async function collectMetadata(client) {
    ...
    return {
        ...
        hasProductDetails: await tableExists(client, 'product_details'),
    };
}
...
async function migrateSchema(client) {
    ...
    await createNormalizedTablesIfMissing(
        ...
        initialMetadata.detailsColumns
    );
    ...
    await replaceLookupTables(
        client,
        metadata.colorsTableExists,
        metadata.sizesTableExists,
