...
function buildVariantSku(parts, fallback = '') {
    const normalized = normalizeSkuPart(
        parts
    ...
    return normalizeSkuPart(fallback);
}

function normalizeBackfillSize(size) {
    if (!size || Array.isArray(size)) {
        return { id: null, name: '' };
    }

    if (typeof size === 'object') {
        const normalizedId = Number(size.id);
...
export function buildBackfillVariants(product, colors = [], sizes = []) {
    ...
    const colorOptions = Array.isArray(colors) && colors.length > 0
        ? colors.map((color) => ({
            id: Number.isInteger(Number(color?.id)) && Number(color.id) > 0 ? Number(color.id) : null,
            name: String(color?.name || '').trim(),
        }))
        : [{ id: null, name: '' }];
    const sizeOptions = Array.isArray(sizes) && sizes.length > 0
        ? sizes.map((size) => normalizeBackfillSize(size))
        : [{ id: null, name: '' }];
    const variants = [];
    ...
    for (const color of colorOptions) {
        for (const size of sizeOptions) {
            const sku = buildVariantSku(
                [baseSku, color.name, size.name],
                `${baseSku}-${displayOrder + 1}`
            ) || `${baseSku}-${displayOrder + 1}`;
...
async function backfillMissingProductVariants(client, metadata) {
    const result = await client.query(`
        SELECT
            p.id,
            p.sku,
            p.price,
            COALESCE(p.colors_id, ARRAY[]::int[]) AS colors_id,
            COALESCE(p.sizes_id, ARRAY[]::int[]) AS sizes_id
        FROM products p
        WHERE NOT EXISTS (
            SELECT 1
            FROM product_variants pv
    ...
    for (const product of result.rows) {
        const colors = await fetchColorsByRefs(
            client,
            product.colors_id,
            metadata.colorsColumns
        );
        const sizeIds = normalizeIdRefs(product.sizes_id);
        const sizeLabels = await fetchSizesByRefs(
            client,
            sizeIds,
            metadata.sizesColumns
        );
        const sizes = sizeLabels.map((name, index) => ({
            id: sizeIds[index] || null,
            name,
        })).filter((size) => size.name);
        const variants = buildBackfillVariants(product, colors, sizes);
...
async function migrateSchema(client) {
    ...
    if (isNormalizedSchema(metadata)) {
        ...
        await backfillMissingProductVariants(client, metadata);
        return;
    }
    ...
    await ensureLookupIndexes(client);
    await backfillMissingProductVariants(client, await collectMetadata(client));
}
