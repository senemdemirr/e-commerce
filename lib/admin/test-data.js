...
let fallbackCampaigns = clone(FALLBACK_CAMPAIGNS);

const FALLBACK_CAMPAIGN_SLIDER_CAMPAIGNS = [
    {
        id: 'demo-1',
...
export function listFallbackCampaignRecords() {
    return clone(fallbackCampaigns);
}

export function incrementFallbackCampaignUsage(id, amount = 1) {
    const normalizedId = Number(id);
    const increment = Math.max(0, Number(amount || 0));

    if (!Number.isInteger(normalizedId) || normalizedId <= 0 || increment <= 0) {
        return null;
    }

    let updatedCampaign = null;

    fallbackCampaigns = fallbackCampaigns.map((campaign) => {
        if (Number(campaign.id) !== normalizedId) {
            return campaign;
        }

        const usageLimit = Number(campaign.usage_limit || 0);
        const hasUsageLimit = Number.isInteger(usageLimit) && usageLimit > 0;
        const currentUsedCount = Math.max(0, Number(campaign.used_count || 0));

        updatedCampaign = {
            ...campaign,
            used_count: hasUsageLimit
                ? Math.min(usageLimit, currentUsedCount + increment)
                : currentUsedCount + increment,
    ...
    return updatedCampaign ? clone(updatedCampaign) : null;
}

export function listFallbackCampaignSliderRecords() {
    return clone(FALLBACK_CAMPAIGN_SLIDER_CAMPAIGNS);
}
